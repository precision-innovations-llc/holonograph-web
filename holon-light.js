...
// holon-light.js — LIGHT-MODE render port of holon.js, for use as a bounded MODULE
// on the white marketing fold. Geometry, cluster logic, section anchors, and all
// interaction are copied verbatim from holon.js; ONLY the render layer changes:
// additive blending → normal blending, bright glow → dark-ink RGBA (fades become
// transparent, not black), UnrealBloom removed, light fog, canvas sized to its
// container instead of the window. The "no pure frame / partial grid" is preserved.
import * as THREE from "three";

// Deterministic PRNG (mulberry32): the marketing hero renders the SAME rich holon
// arrangement on every load, so we can lock a flattering resting angle. Change SEED
// to reseed the whole cube to a new fixed arrangement.
...
const CONFIG = {
  ...
  camDist: 21, fov: 55,
  spin: 0.045, fog: [14, 50],  // LIGHT: slow ambient rotation from the locked rich angle
...
let SECTIONS = Array.from(document.querySelectorAll("#clusterContent .section-slide")).map(function (el) {
  return { title: el.dataset.title, excludeFromNav: el.dataset.excludeNav === "true" };
});
// Module fallback: when the crawlable section markup isn't on the page (e.g. the
// bounded-module prototype), synthesize the section anchors so the cube still gets
// its colored, spread-out interactive clusters.
if (!SECTIONS.length) {
  SECTIONS = ["Observability", "Attribution", "Lens", "Substrate", "Pilot"].map((t) => ({ title: t, excludeFromNav: false }));
}

// ───────────────────────────── LIGHT theme ──────────────────────────────────
// Ink palette: dark, saturated hues that read on paper. toInk() darkens a bright
// (glow-tuned) palette color to an ink while preserving its hue identity, so the
// gold / blue / iris / magenta section distribution survives the light port.
const PAPER = [0.965, 0.961, 0.976];            // matches --paper #f6f5f9
const GRID_INK = [0.40, 0.36, 0.56];            // faint iris-grey for the fragmented grid
function toInk(color) {
  const hsl = { h: 0, s: 0, l: 0 };
  color.getHSL(hsl);
  const c = new THREE.Color().setHSL(hsl.h, Math.min(0.94, hsl.s * 1.08 + 0.14), 0.38);
  return [c.r, c.g, c.b];
}
function inkColor(color) { const i = toInk(color); return new THREE.Color(i[0], i[1], i[2]); }
const clamp01 = (v) => (v < 0 ? 0 : v > 1 ? 1 : v);
const lerp3 = (a, b, t) => [a[0] + (b[0] - a[0]) * t, a[1] + (b[1] - a[1]) * t, a[2] + (b[2] - a[2]) * t];

// ───────────────────────────── boot guards ──────────────────────────────────
const canvas = document.getElementById("holon-lite");
...
function gauss(scale) { return ((srand() + srand() + srand()) / 3 - 0.5) * 2 * scale; }
...
function makeFragmentedGrid(half, divisions, grey, keep) {
  ...
  // LIGHT: grid lines are faint iris-grey ink; the original grey "brightness" maps to alpha.
  const push2 = (a, b, ba, bb) => {
    pos.push(a[0], a[1], a[2], b[0], b[1], b[2]);
    col.push(GRID_INK[0], GRID_INK[1], GRID_INK[2], clamp01(ba * 1.15) * 0.6,
             GRID_INK[0], GRID_INK[1], GRID_INK[2], clamp01(bb * 1.15) * 0.6);
  };
  ...
  g.setAttribute("color", new THREE.Float32BufferAttribute(col, 4)); // RGBA: per-vertex alpha
  return new THREE.LineSegments(g, new THREE.LineBasicMaterial({ vertexColors: true, transparent: true, opacity: 1, depthWrite: false, blending: THREE.NormalBlending }));
}
...
function start() {
  ...
  renderer.setClearColor(0x000000, 0);       // transparent — the white page shows through
  renderer.toneMapping = THREE.NoToneMapping;

  // module viewport = the canvas box (NOT the window), so the cube is a bounded module
  const vw = () => canvas.clientWidth || canvas.width || 1;
  const vh = () => canvas.clientHeight || canvas.height || 1;
  const rectOf = () => canvas.getBoundingClientRect();

  const scene = new THREE.Scene();
  scene.fog = new THREE.Fog(0xf6f5f9, 28, 88); // LIGHT: gentle depth fade; back clusters stay readable
  const camera = new THREE.PerspectiveCamera(CONFIG.fov, 1, 0.1, 100);
  camera.position.set(0, 1.0, CONFIG.camDist);
  const world = new THREE.Group();
  world.rotation.set(0.42, 1.0, 0);  // LIGHT: locked resting angle — rich at first paint for SEED
  world.position.x = 8.0;            // LIGHT: anchored hard-right, bleeding off the right edge
  scene.add(world);
  // LIGHT: no UnrealBloom — bloom is a dark-ground glow effect; we render straight.
  ...
  function pickAnchor(px, py) {
    ...
    const r = rectOf(); px -= r.left; py -= r.top;   // module-relative pointer
    const W = vw(), Hh = vh(), R2 = CONFIG.clusterHitRadius * CONFIG.clusterHitRadius;
  ...
  function rebuild() {
    ...
    for (let ci = 0; ci < CONFIG.clusters; ci++) {
      const nAxis = (srand() * 3) | 0, sign = srand() < 0.5 ? -1 : 1, ip = [0, 1, 2].filter((a) => a !== nAxis);
      ...
      const color = new THREE.Color(set[(srand() * set.length) | 0]);
      ...
      for (let j = 0; j < count; j++) {
        ...
        if (srand() < CONFIG.tendrilFrac) p.addScaledVector(axisVec(nAxis, -sign), rand(0, CONFIG.tendrilReach));
        // snap onto a grid line (2 coords snapped to one of several fine grids)
        const step = CONFIG.snapSteps[(srand() * CONFIG.snapSteps.length) | 0], free = (srand() * 3) | 0, sn = (x) => Math.round(x / step) * step;
    ...
    nodes.forEach((nd, i) => {
      ...
      // LIGHT: brightness → presence. Section nodes read as dark ink; ambient nodes
      // sit close to paper (the old "ambient dim"), so hierarchy survives on white.
      const presence = clamp01(0.4 + (b - 0.5) * 0.32); // ambient stays clearly present; sections darkest
      const rgb = lerp3(PAPER, toInk(nd.color), presence);
      coreCol[i * 3] = rgb[0]; coreCol[i * 3 + 1] = rgb[1]; coreCol[i * 3 + 2] = rgb[2];
    });
    coreBase = coreCol.slice(); // immutable base (highlight layer is disabled in light mode)
    ...
    swarmMat = new THREE.PointsMaterial({ size: CONFIG.coreSize, map: sprite, vertexColors: true, transparent: true, depthWrite: false, blending: THREE.NormalBlending, sizeAttenuation: true });
    ...
    hlMat = new THREE.PointsMaterial({ size: CONFIG.highlightSize, map: sprite, vertexColors: true, transparent: true, depthWrite: false, blending: THREE.NormalBlending, sizeAttenuation: true });
    // LIGHT: cursor-highlight layer is disabled (kept allocated so the loop refs are valid).
    ...
    const addSheath = (from, ax, dir, len, col, b) => {
      ...
      // LIGHT: ink at the node, alpha → 0 at the tip (the fade, made transparent not black)
      const ink = toInk(col), a = Math.min(0.72, b * 0.26);
      shPos.push(from.x, from.y, from.z, e.x, e.y, e.z);
      shCol.push(ink[0], ink[1], ink[2], a, ink[0], ink[1], ink[2], 0);
    };
    for (const nd of nodes) {
      ...
      for (let a = 0; a < arms; a++) addSheath(nd.pos, perps[(srand() * perps.length) | 0], srand() < 0.5 ? 1 : -1, nd.step * rand(0.4, 0.9), nd.color, b * 0.7);
    }
    const shGeo = new THREE.BufferGeometry();
    shGeo.setAttribute("position", new THREE.Float32BufferAttribute(shPos, 3));
    shGeo.setAttribute("color", new THREE.Float32BufferAttribute(shCol, 4)); // RGBA
    world.add(new THREE.LineSegments(shGeo, new THREE.LineBasicMaterial({ vertexColors: true, transparent: true, opacity: 1, depthWrite: false, blending: THREE.NormalBlending })));
    ...
    connectors = [];
    const pushOrg = (a, b, ca, cb) => { orgPos.push(a.x, a.y, a.z, b.x, b.y, b.z); orgCol.push(ca[0], ca[1], ca[2], ca[3], cb[0], cb[1], cb[2], cb[3]); };
    ...
    // LIGHT: stubs/connectors carry ink rgb; the original glow falloff drives alpha.
    const buildStub = (a, end, col) => {
      ...
      for (let s = 0; s <= SEGO; s++) {
        ...
        const c = [ink[0], ink[1], ink[2], Math.min(0.8, glow * 0.72)];
    ...
    const buildConnector = (a, b, ca, cb) => {
      ...
      const inkA = toInk(ca), inkB = toInk(cb);
      let prev = null, prevC = null;
      for (let s = 0; s <= SEGO; s++) {
        const t = s / SEGO; tmpP.lerpVectors(a, b, t);
        const sg = Math.max(0, 1 - t / gap), dg = Math.max(0, (t - (1 - gap)) / gap);
        const w = sg + dg, mix = w > 1e-6 ? dg / w : 0, rgb = lerp3(inkA, inkB, mix);
        const c = [rgb[0], rgb[1], rgb[2], Math.min(0.8, w * B * 0.72)];
        if (prev) pushOrg(prev, tmpP, prevC, c);
        prev = tmpP.clone(); prevC = c;
      }
      connectors.push({ a: a.clone(), b: b.clone(), ca: inkColor(ca), cb: inkColor(cb) });
    };
    for (const ac of anchors) {
      ...
      const pick = () => emitters[(srand() * emitters.length) | 0];
      ...
      for (let r = 0; r < aimed; r++) {
        const o = pick(), tgt = anchors[(srand() * anchors.length) | 0];
    ...
    if (orgPos.length) {
      ...
      orgGeo.setAttribute("color", new THREE.Float32BufferAttribute(orgCol, 4)); // RGBA
      world.add(new THREE.LineSegments(orgGeo, new THREE.LineBasicMaterial({ vertexColors: true, transparent: true, opacity: 1, depthWrite: false, blending: THREE.NormalBlending })));
    }

    // pulses on the connectors
    nS = connectors.length;
    sparks = [];
    for (let i = 0; i < nS; i++) sparks.push({ active: srand() < 0.3, t: 0, wait: rand(0, CONFIG.sparkWait[1]), speedF: rand(0.8, 1.2), phase: srand() * 6.28 });
    ...
    spMat = new THREE.PointsMaterial({ size: CONFIG.sparkSize, map: sprite, vertexColors: true, transparent: true, depthWrite: false, blending: THREE.NormalBlending, sizeAttenuation: true });
  ...
  function resize() {
    const w = vw(), h = vh();
    if (w < 2 || h < 2) return;                 // guard: never hand the renderer a zero-size buffer
    world.position.x = w >= 780 ? 8.0 : 0;      // anchored hard-right on the fold; centered on narrow screens
    renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, 2));
    renderer.setSize(w, h, false);              // drawing buffer = full-bleed stage; CSS controls display
    camera.aspect = w / h; camera.updateProjectionMatrix();
    if (activeIdx >= 0) updateConnector(false); // keep the open panel glued to its cluster
  }
  window.addEventListener("resize", resize);
  if (window.ResizeObserver) { const ro = new ResizeObserver(resize); ro.observe(canvas); }
  ...
  function loop() {
    ...
    if (motion) {
      ...
      if (false && mouseOn && nodeCount) { // LIGHT: cursor-highlight layer disabled
        world.updateMatrixWorld(); camera.updateMatrixWorld();
        const W = vw(), H = vh(), R = CONFIG.highlightRadius, R2 = R * R, HS = CONFIG.highlightStrength;
    ...
    renderer.render(scene, camera); // LIGHT: straight render, no bloom composer
