// Cloudflare Pages Function — /llms-full.txt
//
// Serves the static llms-full.txt template with {{binary}} and {{client}} substituted
...
export async function onRequestGet({ request, env }) {
  const [template, versions, liveClient] = await Promise.all([
    readTemplate(request, env, "/llms-full.txt"),
